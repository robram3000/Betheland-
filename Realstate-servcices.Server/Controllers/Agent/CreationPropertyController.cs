    ...
    public class CreationPropertyController : ControllerBase
    {
        ...
        [FromForm] List<IFormFile>? videos = null)
        {
            try
            {
                ...
                try
                {
                    request = System.Text.Json.JsonSerializer.Deserialize<CreatePropertyRequest>(
                        propertyData,
                        new System.Text.Json.JsonSerializerOptions
                        {
                            PropertyNameCaseInsensitive = true,
                ...
                if (request == null || request.Property == null)
                {
                    ...
                    {
                        Success = false,
                        Message = "Invalid property data format - missing property object"
                    });
                }

                // Validate required fields
                if (string.IsNullOrEmpty(request.Property.Title))
                {
                    Console.WriteLine($"Title validation failed. Title value: '{request.Property.Title}'");
                    Console.WriteLine($"Full property object: {System.Text.Json.JsonSerializer.Serialize(request.Property)}");
                    return BadRequest(new PropertyResponse
                    {
                        Success = false,
