// GlobalAdminTopbar.jsx - COMPLETE REAL-TIME NOTIFICATION VERSION
...
const GlobalAdminTopbar = ({ onToggle, collapsed, mobileView }) => {
    ...
    const enableRealTimeNotifications = useCallback(() => {
        if (!user) return;
        ...
        try {
            ...
            const unsubscribeNew = chatService.onNotificationReceived((newNotification) => {
                ...
                setNotifications(prev => {
                    const mappedNotification = {
                        ...
                        type: mapNotificationType(newNotification.notificationType),
                        rawNotification: newNotification
                    };
            ...
            const unsubscribeCount = chatService.onNotificationCountUpdated((countData) => {
                console.log('🔢 Real-time count update:', countData);
    ...
    // Enhanced real-time notification setup
    useEffect(() => {
        if (user) {
            // Initial load
            loadNotifications();
            ...
            // Enable real-time notifications
            const cleanup = enableRealTimeNotifications();

            // Set up interval for backup updates (every 60 seconds)
            ...
            return () => {
                ...
                chatService.disableRealTimeNotifications();
    ...
    }, [user, loadNotifications, loadNotificationCount, enableRealTimeNotifications]);
