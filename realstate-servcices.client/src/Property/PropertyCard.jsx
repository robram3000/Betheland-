...
import { Card, Tag, Typography, Space, Divider, Badge, Button, message } from 'antd';
import {
    EnvironmentOutlined,
    HeartOutlined,
    HeartFilled,
...
const PropertyCard = ({
    ...
    return (
        <Badge.Ribbon
            text={property.featured ? "Featured" : getStatusText(property.status)}
            color={property.featured ? '#1B3C53' : getStatusColor(property.status)}
            style={{ display: property.featured || property.status !== 'available' ? 'block' : 'none' }}
        >
            <Card
                hoverable
                style={{
